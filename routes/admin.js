...
router.get("/",checkAdmin, async function(req,res){
    ...
    res.render("admin/index", {cinemaCount, movieCount, seatCount, bookingCount});
});
...
router.get("/movieList", checkAdmin, async function (req, res) {
    const movie = await Movies.find().populate("cinemaId");
    // console.log(movie);
    res.render("admin/movie/list", { movies: movie });
});
...
router.get("/seatManagement", checkAdmin, async function (req, res) {
    const cinemas = await Cinemas.find({});
    res.render("admin/seat/seatManagement", { cinemas: cinemas });
});
...
router.get("/bookingmanagement", checkAdmin, async function(req,res){
    const bookings = await Bookings.find().populate("movieId").sort({created: -1});
