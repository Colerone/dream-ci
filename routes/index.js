...
router.get("/moviedetail/:id", async function(req,res){
  const movies = await Movies.findById(req.params.id).populate("cinemaId", "name");
  const time = await Times.findOne({ movieId: req.params.id});
  res.render("moviedetail",{movies : movies, timetable: time.timeTable});
});

router.get("/booking/:id", async function (req,res){
  const movies = await Movies.findById(req.params.id).populate("cinemaId");
  const time = await Times.findOne({movieId : req.params.id});
  console.log(time.timeTable[0])
  res.render("booking",{ movie: movies, time : time});
});
...
router.post("/bookingreceipt", upload.single("receipt"), async function (req,res){
  const update = { updated: Date.now(),};
  if(req.file)update.receipt = "/images/uploads/" + req.file.filename;
  const booking = await Bookings.findByIdAndUpdate(req.body.id,{$set: update,});
